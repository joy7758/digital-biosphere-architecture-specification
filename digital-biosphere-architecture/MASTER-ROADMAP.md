...
| `DP-5B` | 执行 External Developer Trial | DBA + Trial Coordinator | `BLOCKED` | 技术包 `TMAI-DP-v0.1-TRIAL-20260721-001` 已冻结；仍需 3–5 份 session records、失败保留与 Trial Result Report | 处理 `B-008`：补充真实 `participant_source` 后 `DQ-010` 才生效 |
...
| `M2` | 为核心项目确认 canonical status source 与刷新规则 | DBA 协调；各项目提供来源 | `IN_PROGRESS_READY_FOR_REVIEW` | 候选来源 registry、branch/commit snapshot、responsibility chain 与 conflict policy 已形成；Domain Owner 尚未确认 | 人类处理 `DQ-003`；随后决定 `DQ-007` refresh cadence |
...
```text
CURRENT_FOCUS=DP_5B_PARTICIPANT_SOURCE_AND_CONTROLLED_TRIAL
NEXT_BLOCKER=B-008_PARTICIPANT_SOURCE_MISSING
NEXT_DECISION=DQ_009_AFTER_TRIAL_RESULT
NEXT_DELIVERY=THREE_TO_FIVE_EXTERNAL_DEVELOPER_SESSION_RECORDS
NEXT_INTEGRATION=NONE_FEATURE_FREEZE_DURING_TRIAL
