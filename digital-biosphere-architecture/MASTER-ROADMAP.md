...
| `M2` | 为核心项目确认 canonical status source 与刷新规则 | DBA 协调；各项目提供来源 | `GATE_PASSED` | `ADR-021` 确认 source routing、responsibility chain、conflict policy、后果性闸门前人工刷新与 24 小时 `STALE` 阈值 | 保持只读同步；如要启动 `M3`，先独立处理 `DQ-004` |
