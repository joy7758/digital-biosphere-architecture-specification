...
| `DP-5C` | Agent-readable remediation and rerun（智能体可读修复与复测） | DBA | `GATE_PASSED` | `TMAI-ACV-20260722-002`：12/12 sessions、全部阈值、权限安全和负控通过；overall verdict 12/12 `CONDITIONAL` | 保留 `OPEN_WEB_DISCOVERY=NOT_ASSESSED` |
| `DP-R` | Developer Preview Release Decision | DBA + Human Release Authority | `BLOCKED` | Agent Validation `PASS`；仍需 DBOS access decision、`R-015` 复核和 `released_by_ref` | 处理 `B-010` 与 `DQ-009` |
...
```text
CURRENT_FOCUS=DP_R_DBOS_AGENT_ACCESS_RISK_REVIEW_AND_HUMAN_RELEASE_DECISION
NEXT_BLOCKER=B-010_DBOS_AGENT_ACCESS_UNDECIDED
NEXT_DECISION=DBOS_AGENT_ACCESS_OR_DISTRIBUTION_THEN_DQ_009
NEXT_DELIVERY=HUMAN_DBOS_ACCESS_AND_RELEASE_DECISION
