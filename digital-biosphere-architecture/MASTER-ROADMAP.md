...
| `DP-R` | Developer Preview Release Decision | DBA + Human Release Authority | `BLOCKED` | Agent Validation `PASS`；`R-015` 复核完成；仍需 DBOS access decision、开放网络发现限制处理和 `released_by_ref` | 处理 `B-010`、`B-011` 与 `DQ-009` |
...
NEXT_BLOCKER=B-010_DBOS_AGENT_ACCESS_UNDECIDED_AND_B-011_OPEN_WEB_PARTIAL_METADATA_ONLY
NEXT_DECISION=DBOS_AGENT_ACCESS_AND_DISCOVERY_LIMIT_HANDLING_THEN_DQ_009
NEXT_DELIVERY=DISCOVERY_RECHECK_OR_LIMIT_ACCEPTANCE_AND_HUMAN_DBOS_ACCESS_DECISION
