...
| `DP-5D` | Open-web discovery observation（开放网络发现观察） | DBA | `IN_PROGRESS` | `TMAI-OWD-20260722-001=PARTIAL_METADATA_ONLY`；完整 GitHub description 命中，规范英文／中文名称仍未命中 | 规范名称复查或显式接受限制 |
| `DP-R` | Developer Preview Release Decision | DBA + Human Release Authority | `BLOCKED` | Agent Validation `PASS`；`R-015` 复核完成；public-safe wheel 已验证但未发布；仍需分发决定、开放网络发现限制处理和 `released_by_ref` | 处理 `DQ-016`、`B-010`、`B-011` 与 `DQ-009` |
...
```text
CURRENT_FOCUS=DP_5D_DISCOVERY_RECHECK_AND_DP_R_RELEASE_INPUTS
NEXT_BLOCKER=DQ-016_DBOS_WHEEL_DISTRIBUTION_AND_B-011_OPEN_WEB_PARTIAL_METADATA_ONLY
NEXT_DECISION=DQ-016_AND_DISCOVERY_LIMIT_HANDLING_THEN_DQ-009
NEXT_DELIVERY=PUBLIC_SAFE_WHEEL_DECISION_AND_DISCOVERY_RECHECK_OR_LIMIT_ACCEPTANCE
NEXT_INTEGRATION=NONE_UNTIL_ACCESS_DECISION
