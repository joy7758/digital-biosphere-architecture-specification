...
status: human-decisions-recorded-freeze-authorized-immutable-baseline-pending
...
六项精确人工决定已经记录，DBA baseline commit/push 已获授权；当前仍等待 immutable commit/tree
形成。本注册表将四项 reference adoption、`ADR-024` 架构序列、`DQ-018` 有界实现授权和三个仍缺输入的
DQ 汇入一个 truth surface（事实表面），防止版本采纳、实现授权、Git 冻结和部署授权互相替代。
...
IMMUTABLE_BASELINE_CREATED=false
...
8. 本注册表记录决定与 Git 授权，但在 exact commit/tree 出现前仍不是 immutable freeze record。

## 7. Stop Rule（停止规则）

下一条合法路径已收窄为：形成 immutable DBA baseline → fresh DBOS before-state → 只实施 `DQ-018` 精确
slice。`DQ-019`–`DQ-021` 仍等待各自真实输入和独立决定。在不可变提交形成前保持：
