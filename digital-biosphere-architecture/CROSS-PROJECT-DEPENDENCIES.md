...
| `DEP-011` | OpenTelemetry SDK / Collector / OTLP | DBOS | `OBSERVATION_INPUT` | Trace、Metric、Log、Resource 与 Context 未来通过 Telemetry Observation Envelope（遥测观察信封）进入受控准入；DQ-018 已实现无 listener、metadata-only reference boundary 并完成 streaming integrity hardening | `DQ_018_OFFLINE_ADMISSION_STREAMING_HARDENED_PR_G2A_REVIEW_READY_NETWORK_AND_PRODUCTION_BLOCKED` | Version reference、load characterization 或 offline PASS 自动等于 SDK/Collector/runtime adoption；Telemetry 自动成为 Evidence、Verification、Truth、Permission 或 SAEE 评价结论 |
