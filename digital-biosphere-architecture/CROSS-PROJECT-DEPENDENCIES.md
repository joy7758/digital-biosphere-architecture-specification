...
| `DEP-011` | OpenTelemetry SDK / Collector / OTLP | DBOS | `OBSERVATION_INPUT` | DQ-018 无 listener、metadata-only reference boundary 的 current source `ee90c0c…` P001—P005 与 security negatives 已在原生 Linux x86_64 直接验证；DBOS receipt `901bf0d…` 已远端复验 | `DQ_018_SECURITY_REMEDIATED_NATIVE_REMOTE_ATTESTED_HUMAN_REVIEW_READY_NOT_APPROVED_NETWORK_AND_PRODUCTION_BLOCKED` | Version reference、native load characterization、finding closure 或 offline PASS 自动等于 SDK/Collector/runtime adoption；Telemetry 自动成为 Evidence、Verification、Truth、Permission 或 SAEE 评价结论 |
...
| `DEP-018` | DBA human-decision readiness registry | ADR-024 / DQ-018 / DQ-022–DQ-025 / DQ-019–DQ-021 | `GOVERNANCE_GATE` | `HSR-PRE-001/002` 已由 exact source 修复、native validation 和 remote attestation 关闭；2 scoped Medium residuals 保留，Human Security Review 未完成 | `SECURITY_REMEDIATED_PR_G2A_HUMAN_REVIEW_READY_NOT_APPROVED` | 注册表、supplement/attestation、native load PASS、finding closure、0 automated findings 或 Agent recommendation 自动等于 Human Review complete、选择 backend 或授权生产 |
