...
| `DEP-011` | OpenTelemetry SDK / Collector / OTLP | DBOS | `OBSERVATION_INPUT` | Trace、Metric、Log、Resource 与 Context 未来通过 Telemetry Observation Envelope（遥测观察信封）进入受控准入；DQ-018 已实现无 listener、metadata-only reference boundary，并在固定 Linux arm64 环境完成远端绑定验证 | `DQ_018_LINUX_ARM64_REFERENCE_VALIDATED_PR_G2A_REVIEW_READY_NETWORK_AND_PRODUCTION_BLOCKED` | Version reference、load characterization 或 offline PASS 自动等于 SDK/Collector/runtime adoption；Telemetry 自动成为 Evidence、Verification、Truth、Permission 或 SAEE 评价结论 |
...
| `DEP-018` | DBA human-decision readiness registry | ADR-024 / DQ-018 / DQ-022–DQ-025 / DQ-019–DQ-021 | `GOVERNANCE_GATE` | 汇总 ADR/DQ status、human token、依赖与零效果；DQ-022–025、ADR-024、DQ-018 及 Linux arm64 reference supplement 已记录，当前等待 PR-G2A review | `DECISIONS_RECORDED_REMOTE_BASELINES_VERIFIED_PR_G2A_REVIEW_PENDING_NOT_APPROVED` | 注册表或 Agent recommendation 自动关闭 review gate、选择 backend 或授权生产 |
