...
| External Normative Reference | exact tag/commit 变化经 delta inventory 后，在 selected scope 内只含兼容增量或行为澄清 | 自动跟随 `latest`、未区分 Stable/Development、未审查 wire/schema/behavior/security delta 或把 reference adoption 写成 implementation support |
