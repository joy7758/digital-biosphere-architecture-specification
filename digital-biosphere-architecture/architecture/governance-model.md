...
OpenTelemetry SDK、Collector、Backend 及其 Trace、Metric、Log 构成 Observation Plane（观察平面），不是新的治理域。该平面提供可观测性材料并记录采样、丢弃、重试和传输状态；它不拥有 Identity、Evidence、Verification、Evaluation、Decision 或 Permission 的治理权。材料只有经过 DBOS 明确接纳后才可进入存在治理记录。
...
OBSERVATION_PLANE_IS_GOVERNANCE_AUTHORITY=false
