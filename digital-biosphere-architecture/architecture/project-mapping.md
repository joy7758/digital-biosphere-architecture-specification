...
|---|---|---|---|---|---|
| `PROJECT-DIGITAL-BIOSPHERE-ARCHITECTURE` | Digital Biosphere Architecture | Architecture Authority Layer（架构权力层） | Architecture Governance / Rules | 文档仓库不是 Runtime、Entity、最终授权主体或执行权来源；本映射不指派实际 Architecture Authority | 与既有 public meaning layer 的 canonical reconciliation、角色任命和版本发布机制仍待定义 |
...
- Digital Biosphere Architecture → Architecture Authority Layer：维护规则、ADR 和变更流程，不运行 DBOS，不控制 SAEE 算法；
