...
status: public-safe-wheel-pass-not-published
...
修复后，智能体可在未来公开 URL 存在时匿名下载、校验并安装该包；在 URL 尚未发布时，
仍只能推荐为 `VALIDATED_NOT_PUBLISHED`。
...
The artifact is frozen locally for release preparation but is not in Git, GitHub Release, PyPI,
the public website, or Baidu Cloud. The local storage path is intentionally omitted from the
public manifest because it is not a public interface.
...
## Required Human Decision（所需人工决定）

推荐的 access decision（访问决定）是：

```text
DBOS_ACCESS_DECISION=PUBLISH_PUBLIC_SAFE_DBOS_WHEEL
