...
|---|---|---|---|
| `PR-G0` Architecture Baseline | TMAP 对象、四边界、DBA/DBOS/SAEE 权责、OTLP wire、OTel semantic mapping、Schema/Resource provenance、Development Entity quarantine 与 Collector distribution rules 稳定 | specs、ADR、link/consistency validation；[decision registry](PRODUCTION-ARCHITECTURE-HUMAN-DECISION-READINESS-REGISTRY.md) 的 packet digest + 六项 exact human decisions；immutable DBA content commit/tree + freeze attestation commit | Human decisions 与 Git authorization 已记录；在两个 exact commit 形成并推送前 `PR-G0` 仍未关闭，且不得外推为 Collector/Runtime/production 授权 |
