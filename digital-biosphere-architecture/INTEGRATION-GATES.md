...
result=IMPLEMENTED_NATIVE_LINUX_X86_64_VALIDATED_HUMAN_SECURITY_REVIEW_REQUIRED_PR_G2A_NOT_READY_NOT_APPROVED
next_gate=DBA_NATIVE_RECEIPT_REMOTE_ATTESTATION_THEN_HUMAN_SECURITY_REVIEW
```

该记录已从 decision readiness 演进为 implementation receipt 路由；current source 的五项
workload 已在原生 Linux x86_64 形成直接证据并通过 DBOS remote receipt 复验，但 DBA attestation 与 Human Security Review 尚未完成。
...
negative_checks=DBA_SUPPLEMENT_NOT_REMOTE_ATTESTED_HUMAN_SECURITY_REVIEW_INCOMPLETE_23_DQ020_CASES_BLOCKED_NO_LISTENER_NO_COLLECTOR_NO_SAEE_NO_GATE_APPROVAL
...
dba_native_validation_supplement_remote_attested=false
...
current_source_packet_status=NATIVE_VALIDATED_DBOS_RECEIPT_REMOTE_VERIFIED_DBA_ATTESTATION_AND_HUMAN_SECURITY_REVIEW_REQUIRED
result=NOT_READY_NATIVE_VALIDATED_DBA_ATTESTATION_AND_HUMAN_SECURITY_REVIEW_REQUIRED_NOT_APPROVED
...
`NOT_READY_NATIVE_VALIDATED_DBA_ATTESTATION_AND_HUMAN_SECURITY_REVIEW_REQUIRED_NOT_APPROVED` 表示
当前 source 已通过原生 Linux x86_64 直接验证和 DBOS remote receipt 复验，但 DBA supplement
尚未 remote attested，Human Security Review 也未完成，尚不能进入 Human Decision；predecessor
