...
| `DBOS` | `CORE` | Digital Biosphere Operating System | Trusted Multi-Agent Existence Infrastructure（可信多智能体存在基础设施）和 Developer Infrastructure（开发者基础设施） | main `cd3f867...`；DQ-018 source `5c52c1c…`；receipt `aa6440e…`；manifest `fdda745c…` | `DEVELOPER_PREVIEW_RELEASED_DQ_018_OFFLINE_SLICE_PR_G2A_REVIEW_READY` | 离线 Telemetry admission reference slice 已 clean-clone validated；PR-G2A 未批准，PR-G2B/G2T/G2I/G2C 未完成；不得写成 production durability |
