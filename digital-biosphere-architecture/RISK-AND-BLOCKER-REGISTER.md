...
|---|---|---|---|---|---|
| `R-001` | `HIGH` | Program Authority（项目群权力）未指派，重大路线图和准入决定没有稳定 `decided_by_ref` | DBA 现有规范与本章程均记录 `program_authority_assigned=false` | 处理 `DQ-001`，定义角色、复核与替代机制 | `UNASSIGNED` |
...
| `R-013` | `HIGH` | Developer Preview 变更分布在 DBA、DBOS、SAEE 三个未提交且部分高度脏的工作树中，当前 HEAD 不包含本阶段完整实现 | 2026-07-21：DBOS `dirty_count=86`，SAEE `dirty_count=556`，DBA 也有未提交文档 | 精确隔离本阶段文件、审查与提交；从 clean clone 按冻结 source versions 复验 | `BLOCKING_RELEASE` |
...
|---|---|---|---|---|
| `B-001` | 没有已记录的 Program Owner / decision source | 高影响组合准入、优先级冲突和项目群 gate 最终关闭 | `DQ-001` 形成显式决定 | DBA 文档基线和只读审计 |
...
## 4. Cleared Local Blocker（本地已解除阻塞）
