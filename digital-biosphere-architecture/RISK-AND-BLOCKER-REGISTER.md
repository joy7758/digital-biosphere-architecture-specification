...
| `R-042` | `HIGH` | 周期 integrity scan 一次性 materialize 全部 admission records，使长期运行内存随记录量增长并形成可用性风险；并发 measurement observer effect 也可能伪造锁争用 | predecessor `d62b411…` 的 426 个 `TA_STORAGE_UNAVAILABLE` 已定位为 QEMU 下 concurrent global `tracemalloc` 观察者效应；current `ee90c0c…` 在原生 Linux x86_64 的 P001—P005 integrity/source 全部 `PASS`；P003 为 19,199 operations、0 errors，P005 为 3,600 s / 16,797 operations / 59 checks、0 errors；旧错误不删除 | current source 原生五项验证已完成；Windows、production backend、SLO failure/memory revalidation 仍必需 | `NATIVE_LINUX_X86_64_PASS_PRODUCTION_REVALIDATION_REQUIRED` |
...
| `B-019` | production architecture baseline 和 DQ-018 起点尚未远端复验 | DBA predecessor 与 current security-remediation attestation 已形成非循环 remote proof；DBOS source `ee90c0c…` / receipt `901bf0d…` / manifest `306435cd…` 的 40-entry native inventory、538/538 tests、204/204 telemetry、P001—P005 和 `HSR-PRE-001/002` closure 已完成 native Linux x86_64 与 remote clean-clone verification | 关闭 current exact-source provenance 与两个 High finding；Human Security Review complete 和 PR-G2A approval 仍缺失，不产生 approval 或 production authorization |
