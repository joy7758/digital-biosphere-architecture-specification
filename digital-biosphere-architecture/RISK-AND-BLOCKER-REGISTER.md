...
| `R-042` | `HIGH` | 周期 integrity scan 一次性 materialize 全部 admission records，使长期运行内存随记录量增长并形成可用性风险；并发 measurement observer effect 也可能伪造锁争用 | predecessor `d62b411…` 的 426 个 `TA_STORAGE_UNAVAILABLE` 已定位为 QEMU 下 concurrent global `tracemalloc` 观察者效应；successor `23c8f08…` 的 P001—P005 模拟重基线 integrity/source 全部 `PASS`；P003 为 18,953/18,953、0 errors，P005 为 59/59 + final `PASS`；旧错误不删除 | current source 模拟五项重基线已完成；原生 Linux x86_64、Windows、production backend、SLO failure/memory revalidation 仍必需 | `CURRENT_SOURCE_FULL_EMULATED_REBASE_PASS_NATIVE_X86_64_AND_PRODUCTION_REVALIDATION_REQUIRED` |
...
| `B-019` | production architecture baseline 和 DQ-018 起点尚未远端复验 | DBA predecessor 与 Linux arm64 supplement 已有非循环 remote attestation；DBOS current source `23c8f08…` / receipt `b2eac11…` / manifest `43382e42…` 的 packet、15-entry raw inventory、534/534 tests 与 35/35 validators 完成 remote clean clone | 只关闭 current-source packet refresh 与 Agent review remote provenance；原生 x86_64、human security review 和 PR-G2A approval 仍缺失，不产生 readiness/approval 或 production authorization |
