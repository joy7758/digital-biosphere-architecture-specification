...
| `R-042` | `HIGH` | 周期 integrity scan 一次性 materialize 全部 admission records，使长期运行内存随记录量增长并形成可用性风险；并发 measurement observer effect 也可能伪造锁争用 | predecessor `d62b411…` 的 426 个 `TA_STORAGE_UNAVAILABLE` 已定位为 QEMU 下 concurrent global `tracemalloc` 观察者效应；successor `23c8f08…` 在原生 Linux x86_64 的 P001—P005 integrity/source 全部 `PASS`；P003 为 19,200 operations、0 errors，P005 为 3,600 s / 16,787 operations / 59 checks、0 errors；旧错误不删除 | current source 原生五项验证已完成；Windows、production backend、SLO failure/memory revalidation 仍必需 | `NATIVE_LINUX_X86_64_PASS_PRODUCTION_REVALIDATION_REQUIRED` |
...
| `R-044` | `HIGH` | SQLite reference store 接受 group/world-writable 直接父目录，且 path validation 与 `sqlite3.connect` 之间没有稳定 directory capability | direct probe 在 `0777` parent 下成功 initialize；静态 symlink 负例不能覆盖 parent replacement/race | 强制 direct parent ownership/mode contract，增加 writable-parent 与 race negatives；未修复前 PR-G2A fail closed | `OPEN_HSR_PRE_001` |
| `R-045` | `HIGH` | `id_factory` / `clock` callback 输出未经 reference、敏感 marker、timestamp 与长度复核即可持久化 | direct probe 使 sensitive reference canary 和 invalid secret-bearing timestamp 进入 receipt/record/store | 所有生成 reference/timestamp 在 persistence boundary 重新验证，增加 canary 负例；未修复前 PR-G2A fail closed | `OPEN_HSR_PRE_002` |
...
| `B-019` | production architecture baseline 和 DQ-018 起点尚未远端复验 | DBA predecessor 与 native supplement 已有非循环 remote attestation；DBOS current source `23c8f08…` / native receipt `ff1752c…` / manifest `43382e42…` 的 32-entry raw inventory、534/534 tests、200/200 telemetry 和 P001—P005 已在原生 Linux x86_64 完成 remote clean clone verification | 只关闭 native receipt 与 DBA supplement provenance；Human Security Review 和 PR-G2A approval 仍缺失，不产生 readiness/approval 或 production authorization |
