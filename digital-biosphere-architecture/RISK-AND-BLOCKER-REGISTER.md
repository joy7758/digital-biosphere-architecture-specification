...
| `R-042` | `HIGH` | 周期 integrity scan 一次性 materialize 全部 admission records，使长期运行内存随记录量增长并形成可用性风险；并发 measurement observer effect 也可能伪造锁争用 | predecessor `d62b411…` 的 426 个 `TA_STORAGE_UNAVAILABLE` 已定位为 QEMU 下 concurrent global `tracemalloc` 观察者效应；successor `23c8f08…` 同介质 300s / 32 workers 为 18,953/18,953、0 errors、integrity/source `PASS`，并保留 ru_maxrss；旧错误不删除 | observer effect 已在当前 P003 缓解；但 P001/P002/P004/P005 当前源码重基线、原生 Linux x86_64、Windows、production backend、SLO failure/memory revalidation 仍必需 | `P003_OBSERVER_EFFECT_MITIGATED_CURRENT_SOURCE_FULL_REBASE_AND_NATIVE_X86_64_REQUIRED` |
...
| `B-019` | production architecture baseline 和 DQ-018 起点尚未远端复验 | DBA predecessor 与 Linux arm64 supplement 已有非循环 remote attestation；DBOS current source `23c8f08…` / receipt `759b69c…` / manifest `43382e42…` 的 P003 observer-effect 回执完成 remote clean clone | 只关闭 current P003 remote provenance；P001/P002/P004/P005 当前源码重基线和原生 x86_64 仍缺失，不产生 PR-G2A readiness/approval 或 production authorization |
