...
| `R-021` | `HIGH` | GenAI/MCP Semantic Conventions 仍为 Development 或缺少稳定 Schema URL，自动跟随上游会破坏长期合同 | 2026-07-22 `semantic-conventions-genai@2e994c6...` README 为 `Schema URL: TODO` | 47-field/9-source exact commit mapping、隔离 Development lane、兼容声明、回放测试和 fail closed；`DQ-023` 仍待人工决定 | `MAPPING_CONTRACT_DEFINED_DQ_023_PENDING_IMPLEMENTATION` |
...
| `R-025` | `CRITICAL` | 单机 JSON、`fcntl`、atomic replace 或 SQLite reference pass 被误写成 production durability，导致跨 store 崩溃不一致或无法恢复 | 当前 Authorization/Registration/Receipt 各自原子写入，但无 canonical transaction、HA、PITR 或 RTO/RPO 证据 | `DQ-018` 只证明 reference contract；[`production-persistence-adapter-specification.md`](architecture/production-persistence-adapter-specification.md) 要求 `DQ-019`、HA/fencing、backup+WAL/PITR、restore 和迁移证据 | `OPEN_IMPLEMENTATION_NOT_AUTHORIZED` |
...
| `R-033` | `CRITICAL` | `service.instance.id`、`gen_ai.agent.id`、trace/span/conversation ID、tool 或 evaluation telemetry 被提升为 DBOS Identity/Execution/Capability/Permission 或 SAEE/Governance Authority | OTel/GenAI 字段为资源、相关性、操作或评价材料；来源可能由 instrumentation/Collector 提供或伪造 | strict observation schema 拒绝 authority namespace/field；DBOS binding 只接受独立 context + digest；47-field authority rule；46-case catalog 中 7 个 authority cases 和相关 source/content/drift cases；当前只完成 architecture rehearsal，validator/implementation 待授权 | `MITIGATED_BY_PROPOSED_CONTRACT_DQ_023_AND_IMPLEMENTATION_PENDING` |
...
| `R-040` | `CRITICAL` | 已记录 reference adoption、Git freeze、bounded implementation authorization 和未来 blanket wording 互相替代 | 六项不同 exact token、Authority 和效果已独立记录；未来新版本/部署/外部资源仍没有 decision | registry 绑定 13 个 decision-input digests、9 个独立 entries、dependency DAG 与 token/decider/time；禁止把“以后按推荐”扩展到新决策对象 | `HUMAN_DECISIONS_RECORDED_IMMUTABLE_FREEZE_PENDING` |
...
| `B-013` | DBOS 尚无 OTel/OTLP intake、Telemetry admission ledger、canonical production durability、backup/restore 与 SLO 自观测 | `PR-G2`、`PR-G3`、`PR-G5` 和 bounded pilot | 经 `DQ-018` 授权并分阶段实现协议符合性、耐久性、恢复、故障和 SLO 证据 | DBA 规范维护与 exact slice 决策审查 |
...
| `B-018` | Schema/Resource reference 与 OTel Entity quarantine 已由 `DQ-024` 采纳，但 acquisition/cache/transform、Resource provenance 和 data-governance validator 未实现，45 cases 执行为 0 | `PR-G2`、`PR-G3`、`PR-G5`、`PR-G6` 的 Schema/Resource/Entity provenance | 相应 implementation/deployment authorization + exact source/policy/environment binding + 45/45 case-level result + independent review | DBA 的 source/profile/decision review；不授权修改 DBOS/SAEE |
| `B-019` | 六项 human token/decider/time 与 DBA commit/push 授权已记录，但 production worktree 尚未形成 immutable content commit/tree 和 freeze attestation | `PR-G0` freeze 与 `DQ-018` 实施起点 | 完成验证、content commit、freeze attestation commit 和 remote push，并在 fresh DBOS before-state 中绑定 exact SHA | 不阻塞只读验证；在 freeze 前仍阻止 DBOS 实施起跑 |
