...
status: observed-governance-snapshot-production-human-decisions-and-immutable-baseline-pending
...
production_readiness_status: reference-and-sequence-decisions-recorded-dq-018-authorized-immutable-baseline-pending-not-production-ready
...
CURRENT_MILESTONE=IMMUTABLE-DBA-BASELINE-THEN-DQ-018-TELEMETRY-ADMISSION-FOUNDATION
CURRENT_ARCHITECTURE_MILESTONE=REFERENCE-AND-STAGED-SEQUENCE-DECISIONS-RECORDED
CURRENT_GOVERNANCE_GATE=IMMUTABLE-DBA-BASELINE-COMMIT-AND-PUSH
CORE_PROJECTS=3
PILOT_PROJECTS=1
...
PRODUCTION_MAPPING_REMOTE_BASELINE_CREATED=false
...
Developer Preview v0.1 的历史发布结果 `PASS_WITH_DISCLOSED_LIMITATIONS` 只表示其发布闸门通过。当前 `PROGRAM_HEALTH=PASS_WITH_PRODUCTION_GATES_OPEN` 只表示生产架构基线和 `PR-G1` 只读实现映射通过，可进入精确实施切片的人工决策；二者都不表示生产就绪、客户采用、完整端到端集成或开放网络自然发现通过。
...
| OpenTelemetry Observability Profile | `PROFILE_DEFINED_NOT_IMPLEMENTED` | OTel `1.59.0`、OTLP candidate `v1.11.0@790608c...`、Semantic Conventions `1.43.0` 与 GenAI exact commit 的参考边界已记录；OTLP version delta 与两路评审完成，但 `DQ-022` 尚未采纳；未部署 Collector 或 endpoint |
| OpenTelemetry Semantic Mapping and Conformance | `DQ_023_REFERENCE_ADOPTED_NOT_IMPLEMENTED_NOT_EXECUTED` | OTel Spec `v1.59.0@a824fb4...`、core SemConv `v1.43.0@89aae438...` 与 GenAI `2e994c6d...` 47-field/7-group/9-source 双层映射已采纳为参考；46-case 目录/结果 Schema 与 12/12 + 23/23 负例通过；validator 未实现、46 cases 执行为 0 |
| OpenTelemetry Schema / Resource / Entity Provenance | `DQ_024_REFERENCE_AND_QUARANTINE_ADOPTED_NOT_IMPLEMENTED_NOT_EXECUTED` | exact OTel `v1.59.0` Schema/Resource rules、core schema `1.43.0` digest 与 Development Entity quarantine 已采纳；7 组 45 cases、2/2 Schema、3/3 positive fixtures、14/14 + 37/37 negatives；validator 未实现、45 cases 执行为 0 |
...
| Production Human Decision Readiness | `REGISTRY_SCHEMA_VALID_DECISIONS_NOT_RECORDED_BASELINE_NOT_FROZEN` | 8 个 DQ + `ADR-024`、13 个 decision-input digest、dependency DAG、single-token rule 与全零效果已机器登记；本轮千帆优先 `C`、方舟优先 `D`，共同拒绝当前 production recommendation 和继续新增技术合同；注册表不创建 Decision 或 Git freeze |
| Production Readiness | `DQ_018_READY_FOR_HUMAN_DECISION` | `PR-G1` 已通过；DQ-018 与完整 staged production sequence 均获两路智能体次序推荐，同时两路均拒绝当前 production recommendation；production persistence、OTLP/Collector、Evidence admission 规范已拟议，`DQ-019`–`DQ-021` 仍 `BLOCKED_INPUT`；所有实施、SAEE isolation、安全恢复、Pilot 和生产授权均未通过 |
...
- `DQ-018`：`DQ-018=AUTHORIZE_DBOS_PR_2A_TELEMETRY_ADMISSION_FOUNDATION` 已由 `zhangbin` 决定，`dbos_domain_owner_ref=zhangbin`；授权只覆盖 exact offline admission foundation，仍等待 immutable DBA baseline 和 fresh DBOS before-state 才可开始；
...
- `DQ-023`：core SemConv `v1.43.0` 与 pinned GenAI Development mapping 已采纳；validator 未实现且 46 cases 执行为 0；
- `DQ-024`：Schema/Resource `v1.59` reference 与 Development Entity quarantine 已采纳；OTel Entity 仍不能成为 DBOS Digital Entity，45 cases 执行为 0；
...
- Production Human Decision Readiness Registry：六项 exact token、`decided_by_ref`、时间、Git authorization 与 13 个 decision-input digest 已对账并通过 Schema；immutable baseline 仍为 false；
...
- `B-013`：DBOS 尚无 OTel/OTLP intake、Telemetry admission ledger、canonical production durability 与恢复证据；
...
1. 在 `codex/production-observability-baseline` 创建并推送 immutable DBA content commit 与 freeze attestation commit；
2. 绑定 exact DBA commit/tree，保留 `PR-G1` 的 public remote、development observation 与 artifact source 三层事实；
3. 在独立 DBOS worktree 对 exact `origin/main` 执行 fresh before-state，必须使 334 tests 与 34 validators fail-closed 通过；
4. before-state 通过后，只实施 `DQ-018` exact slice，并运行新增 conformance/fault/negative/recovery tests；
5. 不修改 SAEE，不构建 Collector binary/image，不创建 Collector config、endpoint、Runtime、Evidence、Entity 或 Permission；
6. `DQ-019`–`DQ-021` 仅在各自 prerequisite 具备后进入审查；
7. 在 `PR-G2` 至 `PR-G7` 通过前保持 `PRODUCTION_READY=false`。
