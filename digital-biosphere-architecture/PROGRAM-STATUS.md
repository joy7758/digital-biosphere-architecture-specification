...
status: observed-governance-snapshot-dq-018-security-precheck-open-high-findings-pr-g2a-not-ready
...
production_readiness_status: dq-018-native-validated-security-remediation-required-pr-g2a-not-ready-full-pr-g2-blocked-not-production-ready
...
CURRENT_MILESTONE=DBOS-PR-G2A-HUMAN-SECURITY-REVIEW-PREPARATION
CURRENT_ARCHITECTURE_MILESTONE=DQ-018-NATIVE-LINUX-X86_64-DIRECT-VALIDATION-PASS
CURRENT_GOVERNANCE_GATE=PR-G2A-SECURITY-PRECHECK-BLOCKED-REMEDIATION-OR-SCOPED-RISK-ACCEPTANCE-REQUIRED
CORE_PROJECTS=3
PILOT_PROJECTS=1
...
DBOS_DQ018_CURRENT_SOURCE_PACKET_STATUS=NATIVE_VALIDATED_DBA_SUPPLEMENT_REMOTE_ATTESTED_HUMAN_SECURITY_REVIEW_REQUIRED
...
DBA_DQ018_SECURITY_PRECHECK_OPEN_HIGH_FINDINGS=2
...
| `DBOS` | `main` + `codex/telemetry-admission-foundation` | main `cd3f867…`; DQ-018 source `23c8f08…`; native receipt `ff1752c…` | main 保持 Developer Preview；生产分支只实现离线 Telemetry Admission reference slice | native Linux x86_64 fresh remote validation 为 534/534 tests、200/200 telemetry、25/25 Telemetry validator；隔离 validator 为 30 PASS + 5 expected external-source fail closed；`TA-P001`—`TA-P005`、rollback 和 reproducible wheel 均 `PASS` | native direct validation 与 DBA remote attestation 已完成；Human Security Review 尚未完成，PR-G2A 仍未 review-ready 且未批准 |
...
| OpenTelemetry Observability Profile | `DQ_018_NATIVE_LINUX_X86_64_DIRECT_VALIDATION_PASS_HUMAN_SECURITY_REVIEW_REQUIRED_PR_G2A_NOT_READY` | DQ-022–025 exact references 已采纳；当前 DBOS source 的 P001—P005 已在原生 Linux x86_64 完成直接验证并远端复验；未部署 Collector 或 endpoint |
...
| Production Human Decision Readiness | `DQ_018_SECURITY_PRECHECK_BLOCKED_OPEN_HIGH_FINDINGS_PR_G2A_NOT_READY` | native Linux x86_64 direct validation、DBOS receipt 与 DBA supplement 已远端复验；安全预审发现 2 个 open High findings 和 2 个 scoped Medium residuals，Human Security Review 未完成 |
| Production Readiness | `DQ_018_NATIVE_VALIDATED_HUMAN_SECURITY_REVIEW_REQUIRED_PR_G2A_NOT_READY_FULL_PR_G2_BLOCKED` | 当前 source 的原生 P001—P005、rollback 和 reproducible wheel 全部 `PASS`；但 PR-G2A Human Security Review、approval 和后继 gates 均未完成 |
...
- `DQ-018`：当前 exact source `23c8f08…`、native receipt `ff1752c…`、manifest `43382e42…`；原生验证与远端证明均完成。安全预审进一步发现 `HSR-PRE-001/002` 两个 open High findings（父目录信任约束、生成 reference/clock 持久化验证）和两个 scoped Medium residuals；Human Security Review 尚未完成，不能进入 Human Decision；
...
1. 对 `HSR-PRE-001/002` 实施 DQ-018 scope 内的最小安全加固并增加负例，或形成显式、限域、带期限的 Human Risk Acceptance；
2. 精确 source 变化后重跑全仓、security、native Linux x86_64 和 remote attestation，再完成独立 Human Security Review；
