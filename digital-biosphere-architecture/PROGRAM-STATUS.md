...
status: observed-governance-snapshot-dq-018-current-source-full-emulated-load-rebase-pass-native-required-pr-g2a-not-ready
...
CURRENT_MILESTONE=DBOS-PR-G2A-CURRENT-SOURCE-PACKET-REFRESH
CURRENT_ARCHITECTURE_MILESTONE=DQ-018-CURRENT-SOURCE-FULL-EMULATED-WORKLOAD-REBASE-PASS-NATIVE-X86-64-REQUIRED
CURRENT_GOVERNANCE_GATE=PR-G2A-CURRENT-SOURCE-PACKET-REFRESH
CORE_PROJECTS=3
PILOT_PROJECTS=1
...
DQ_018_CURRENT_SOURCE_TA_P005_ONE_HOUR_REEXECUTED=false
...
| `DBOS` | `main` + `codex/telemetry-admission-foundation` | main `cd3f867…`; DQ-018 source `23c8f08…`; receipt `3f5b848…` | main 保持 Developer Preview；生产分支只实现离线 Telemetry Admission reference slice | fresh remote clean clone 为 534/534 tests、200/200 telemetry、30 validators PASS + 5 external-source fail closed、0 unexpected；当前源码 `TA-P001`—`TA-P005` 模拟重基线 integrity/source 全部 `PASS`，authority effects 全部 false | 当前五项模拟重基线完成；原生 x86_64 未验证，PR-G2A 当前源码包仍未就绪且未批准 |
...
| Production Human Decision Readiness | `REFERENCE_DECISIONS_RECORDED_DQ_018_EXECUTED_PR_G2A_CURRENT_SOURCE_PACKET_REFRESH_REQUIRED` | DQ-022–025、ADR-024 与 DQ-018 已记录并远端追溯；当前源码五项模拟重基线已形成，但 native x86_64 和新的 independent review packet 缺失，不能进入 PR-G2A Human Review |
...
- `DQ-018`：当前 exact source `23c8f08…`、receipt `3f5b848…`、manifest `43382e42…`；534/534 tests、200/200 telemetry tests、30 validators PASS + 5 external-source fail closed、0 unexpected；`TA-P001`—`TA-P005` 模拟重基线 integrity/source 全部 `PASS`，authority effects 全部 false。原生 x86_64 尚未验证；因此 `PR-G2A` 当前源码审查包仍需要刷新，尚不具备 Human Review 输入完整性；
...
2. 原生 Linux x86_64 证据就绪后刷新 current-source PR-G2A packet，再进入独立 Human Review；
