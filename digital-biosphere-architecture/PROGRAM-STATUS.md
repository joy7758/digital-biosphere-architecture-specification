...
status: observed-governance-snapshot-dq-018-x86-64-emulated-partial-supplement-remote-attested-pr-g2a-pending
...
production_readiness_status: dq-018-linux-arm64-native-pass-x86-64-emulated-partial-pr-g2a-pending-full-pr-g2-blocked-not-production-ready
...
CURRENT_ARCHITECTURE_MILESTONE=DQ-018-LINUX-ARM64-NATIVE-PASS-X86-64-EMULATED-PARTIAL-SUPPLEMENT-REMOTE-ATTESTED
...
DBOS_DQ018_LINUX_AMD64_EMULATED_COMPATIBILITY=PARTIAL
DBOS_DQ018_LINUX_AMD64_EMULATED_SUPPLEMENT_REMOTE_ATTESTED=true
...
DQ_018_EXACT_SOURCE_COMMIT=d62b411845b18af3ec79af67622666e96d1baed3
...
| `DBOS` | `main` + `codex/telemetry-admission-foundation` | main `cd3f867…`; DQ-018 source `d62b411…`; receipt `8c6ddd7…` | main 保持 Developer Preview；生产分支只实现离线 Telemetry Admission reference slice | Linux arm64 原生参考 PASS；当前 amd64 模拟 clean clone 为 534/534 tests、200/200 telemetry、30 validators PASS + 5 external-source fail closed、0 unexpected；`TA-P003` 有 426 个 storage errors，overall `PARTIAL` | x86_64 模拟兼容 supplement 已远端证明，但原生 x86_64 未验证且 `PR-G2A` 未批准；完整 `PR-G2` 与 production ready 均为 false |
...
| OpenTelemetry Observability Profile | `DQ_018_LINUX_ARM64_NATIVE_PASS_X86_64_EMULATED_PARTIAL_REMOTE_ATTESTED_PR_G2A_REVIEW_READY` | DQ-022–025 exact references 已采纳；DBOS 无 listener 的 metadata-only 准入切片在 macOS arm64、固定 Linux arm64 原生环境有 PASS 回执；amd64 模拟兼容 `PARTIAL` 回执及 DBA supplement 已远端复验；未验证原生 x86_64，未部署 Collector 或 endpoint |
...
| Production Readiness | `DQ_018_LINUX_ARM64_NATIVE_PASS_X86_64_EMULATED_PARTIAL_PR_G2A_REVIEW_READY_FULL_PR_G2_BLOCKED` | exact offline slice 已通过固定 Linux arm64 原生 clean clone；amd64 模拟环境功能/安全/一小时完整性通过但 32-worker case 有 426 个 storage errors，原生 x86_64 未验证；PR-G2A 未批准，后继 gates 均未完成 |
...
- `DQ-018`：当前 exact source `d62b411…`、receipt `8c6ddd7…`、manifest `6d1bcbbf…`；534/534 tests、200/200 telemetry tests、30 validators PASS + 5 external-source fail closed、0 unexpected；amd64 模拟 `TA-P005` 一小时 59/59 + final integrity PASS，但 `TA-P003` 有 426 个 `TA_STORAGE_UNAVAILABLE`，所以仅 `PARTIAL` 且原生 x86_64 未验证；Linux arm64 原生 predecessor PASS 与 x86_64 模拟 supplement remote attestation 均已记录；当前仍等待 `PR-G2A Human Review`；
...
1. 对 exact source `d62b411…`、receipt `8c6ddd7…`、manifest `6d1bcbbf…`、hardened packet、Linux arm64 原生 supplement 和 x86_64 模拟 `PARTIAL` supplement 执行 `PR-G2A Human Review`；原生 x86_64 仍须重跑；
