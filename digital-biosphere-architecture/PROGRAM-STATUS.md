...
- `DQ-018`：exact token 与 `dbos_domain_owner_ref=zhangbin` 已记录，DBA content commit/tree 已冻结；授权只覆盖 offline admission foundation，仍等待 remote baseline verification 和 fresh DBOS before-state 才可开始；
...
- Production Human Decision Registry：六项 exact token、`decided_by_ref`、时间、Git authorization 与 13 个 decision-input digest 已对账并通过 Schema；content commit/tree 已冻结，remote verification 待完成；
...
1. 推送 `codex/production-observability-baseline`，并从远端复验 content commit 与 freeze attestation commit；
