...
status: observed-governance-snapshot-dq-018-p003-observer-effect-hardened-current-source-rebase-required-pr-g2a-not-ready
...
production_readiness_status: dq-018-current-source-p003-pass-full-workload-rebase-incomplete-pr-g2a-not-ready-full-pr-g2-blocked-not-production-ready
...
CURRENT_ARCHITECTURE_MILESTONE=DQ-018-P003-OBSERVER-EFFECT-HARDENED-FULL-WORKLOAD-REBASE-INCOMPLETE
...
DBOS_DQ018_CURRENT_SOURCE_FULL_WORKLOAD_REBASE_COMPLETE=false
...
PR_G2A_P003_OBSERVER_EFFECT_SUPPLEMENT_REMOTE_ATTESTED=false
...
| `DBOS` | `main` + `codex/telemetry-admission-foundation` | main `cd3f867…`; DQ-018 source `23c8f08…`; receipt `759b69c…` | main 保持 Developer Preview；生产分支只实现离线 Telemetry Admission reference slice | fresh remote clean clone 为 534/534 tests、200/200 telemetry、30 validators PASS + 5 external-source fail closed、0 unexpected；当前源码 `TA-P003` 模拟 tmpfs 18,953/18,953、0 errors、integrity/source `PASS` | P003 观察者效应已修正；当前 manifest 的 P001/P002/P004/P005 尚未重跑，原生 x86_64 未验证，PR-G2A 当前源码包未就绪且未批准 |
...
| OpenTelemetry Observability Profile | `DQ_018_CURRENT_SOURCE_P003_PASS_FULL_WORKLOAD_REBASE_INCOMPLETE_PR_G2A_NOT_READY` | DQ-022–025 exact references 已采纳；当前 DBOS source 的 P003 模拟并发观察者效应已修正并远端复验；P001/P002/P004/P005 仍绑定 predecessor，不能迁移为当前结果；未验证原生 x86_64，未部署 Collector 或 endpoint |
...
| Production Human Decision Readiness | `REFERENCE_DECISIONS_RECORDED_DQ_018_EXECUTED_PR_G2A_CURRENT_SOURCE_PACKET_REFRESH_REQUIRED` | DQ-022–025、ADR-024 与 DQ-018 已记录并远端追溯；当前源码完整 workload packet 尚未形成，不能进入 PR-G2A Human Review |
| Production Readiness | `DQ_018_CURRENT_SOURCE_P003_PASS_FULL_WORKLOAD_REBASE_INCOMPLETE_PR_G2A_NOT_READY_FULL_PR_G2_BLOCKED` | 当前 source 的 P003 模拟 32-worker run 已达 18,953 次成功、0 errors；但完整 P001—P005 当前源码重基线、原生 x86_64、PR-G2A approval 和后继 gates 均未完成 |
...
- `DQ-018`：当前 exact source `23c8f08…`、receipt `759b69c…`、manifest `43382e42…`；534/534 tests、200/200 telemetry tests、30 validators PASS + 5 external-source fail closed、0 unexpected；`TA-P003` 模拟 tmpfs 为 18,953/18,953、0 errors、integrity/source `PASS`。但 P001/P002/P004/P005 尚未在当前 manifest 重跑，predecessor 五项结果不能自动迁移，原生 x86_64 也未验证；因此 `PR-G2A` 当前源码审查包需要刷新，尚不具备 Human Review 输入完整性；
...
1. 在 exact source `23c8f08…`、receipt `759b69c…`、manifest `43382e42…` 上重跑 P001/P002/P004/P005，形成 current-source P001—P005 完整重基线；
2. 完整重基线和原生 Linux x86_64 证据就绪后刷新 PR-G2A packet，再进入独立 Human Review；
