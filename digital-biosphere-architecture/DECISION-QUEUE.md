...
| `PR-G2A` | `P0` | 是否批准 DQ-018 exact hardened offline Telemetry Admission reference slice 通过阶段闸门？ | `CURRENT_SOURCE_PACKET_REFRESH_REQUIRED_REMOTE_LOAD_REBASE_VERIFIED_NOT_READY_NOT_APPROVED` | [current-source load rebase supplement](PR-G2A-CURRENT-SOURCE-LOAD-REBASE-SUPPLEMENT-2026-07-23.md) / [JSON](PR-G2A-CURRENT-SOURCE-LOAD-REBASE-SUPPLEMENT-2026-07-23.json)；DBOS source `23c8f08…` / receipt `3f5b848…`；P003 supplement/attestation 与 predecessor packet 只作历史输入 | 当前不作 Human Decision；原生 x86_64 证据和新的 independent review packet 形成后才能重新进入 review-ready | full emulated load rebase、旧 packet、534 tests 或 Agent recommendation 自动等于 Human Approval/native validation |
