...
- 更新并提交 PR-G2A review packet，使其绑定 exact source commit、独立技术审查和 clean-clone receipts；
...
Commit/push 只使源码远端可追溯，不产生 PR-G2A Human Approval、完整 PR-G2、DQ-019、
DQ-020、DQ-021、部署或生产状态。
...
5. clean clone 任一必需验证失败则停止在当前阶段，不更新为 PR-G2A review ready；
...
DBOS_HISTORICAL_SOURCE_COMMIT=a3e491c954e911d84baa09fd6d9466fb80280941
DBOS_HARDENED_SOURCE_COMMIT=5c52c1c2f44767c0b13b4ac9670425721b9ea0dd
...
加固源码 commit 与 receipt-only commit 已分别以非强制 push 推送到远端同名分支。远端精确
源码干净检出结果包括：531/531 tests、197/197 telemetry tests、35/35 validators、25/25
telemetry validator checks、OTel authorized subset 32/32 + 36/36、rollback PASS、7 个修复发现、
8 个新增负向／恢复测试，以及两个独立远端检出的 byte-identical wheel。
...
完整 `PR-G2` 仍由 `PR-G2B`、`PR-G2T`、`PR-G2I`、`PR-G2C` 阻塞。
